...
Timeline.OriginalEventPainter.prototype.paintImpreciseInstantEvent=function(M,R,V,S){var W=this._timeline.getDocument();
var K=M.getText();
var G=M.getStart();
var T=M.getEnd();
var H=Math.round(this._band.dateToPixelOffset(G));
var B=Math.round(this._band.dateToPixelOffset(T));
var A=Math.round(H+R.iconWidth/2);
var D=Math.round(H-R.iconWidth/2);
var F=this._frc.computeSize(K);
var L=A+V.event.label.offsetFromLine;
var I=L+F.width;
var P=Math.max(I,B);
var O=this._findFreeTrack(P);
var Q=Math.round(R.trackOffset+O*R.trackIncrement+R.trackHeight/2-F.height/2);
var C=this._paintEventIcon(M,O,D,R,V);
var U=this._paintEventLabel(M,K,L,Q,F.width,F.height,V);
var E=this._paintEventTape(M,O,H,B,V.event.instant.impreciseColor,V.event.instant.impreciseOpacity,R,V);
var N=this;
var J=function(Y,X,Z){return N._onClickInstantEvent(C.elmt,X,M);
};
SimileAjax.DOM.registerEvent(C.elmt,"mousedown",J);
SimileAjax.DOM.registerEvent(E.elmt,"mousedown",J);
SimileAjax.DOM.registerEvent(U.elmt,"mousedown",J);
this._createHighlightDiv(S,C,V);
...
Timeline.OriginalEventPainter.prototype._paintEventTape=function(K,B,D,A,G,C,I,H){var F=A-D;
var E=H.event.tape.height;
var J=I.trackOffset+B*I.trackIncrement;
var L=this._timeline.getDocument().createElement("div");
L.className="timeline-event-tape";
L.style.left=D+"px";
L.style.width=F+"px";
L.style.top=J+"px";
if(K._title!=null){L.title=K._title;
}SimileAjax.Graphics.setOpacity(L,C);
this._eventLayer.appendChild(L);
return{left:D,top:J,width:F,height:E,elmt:L};
};
...
F<G.events.length;
F++){var A=G.events[F];
var K=new Timeline.DefaultEventSource.Event(("id" in A)?A.id:undefined,D(A.start),D(A.end),D(A.latestStart),D(A.earliestEnd),A.isDuration||false,A.title,A.description,this._resolveRelativeURL(A.image,C),this._resolveRelativeURL(A.link,C),this._resolveRelativeURL(A.icon,C),A.color,A.textColor,A.classname);
K._obj=A;
...
}var L=new Timeline.DefaultEventSource.Event(C["id"],F(C["start"]),F(C["end"]),F(C["latestStart"]),F(C["earliestEnd"]),C["isDuration"]!="true",C["title"],C["description"],this._resolveRelativeURL(C["image"],D),this._resolveRelativeURL(C["link"],D),this._resolveRelativeURL(C["icon"],D),C["color"],C["textColor"],C["classname"]);
L._bindings=C;
...
Timeline.DefaultEventSource.Event=function(C,B,H,J,D,A,O,N,E,L,K,G,M,F,I){C=(C)?C.trim():"";
this._id=C.length>0?C:("e"+Math.floor(Math.random()*1000000));
this._instant=A||(H==null);
this._start=B;
this._end=(H!=null)?H:B;
this._latestStart=(J!=null)?J:(A?this._end:this._start);
this._earliestEnd=(D!=null)?D:(A?this._start:this._end);
...
this._classname=(I!=null&&I!="")?I:null;
this._wikiURL=null;
